...
# test 4 tasks
declare -a tasks=('ShadowHandOver' 'ShadowHandBottleCap' 'ShadowHandPen' 'ShadowHandCatchOver2Underarm'
)
...
for j in ${!seeds[@]}; do
	for i in ${!tasks[@]}; do
		...
		nohup python train.py --task=${tasks[$i]} --seed=${seeds[$j]} --rl_device=cuda:$((2 + i % 6)) --sim_device=cuda:$((2 + i % 6)) --graphics_device_id=$((2 + i % 6)) --algo=ppo --learned_seed=$(seq -s, ${seeds[0]} 1 ${seeds[$j]}) --headless --wandb_activate=True --wandb_entity=quantumiracle >> log/$DATE/${tasks[$i]}.log &
		# fine-tune
		# echo nohup python train.py --task=${tasks[$i]} --seed=${seeds[$j]} --model_dir=./logs/${tasks[$i]}/ppo/ppo_seed$((${seeds[$j]}))/model_40000.pt --rl_device=cuda:$((2 + i % 6)) --sim_device=cuda:$((2 + i % 6)) --graphics_device_id=$((2 + i % 6)) --algo=ppo --headless --wandb_activate=True --wandb_entity=quantumiracle : log/$DATE/${tasks[$i]}.log &	
		# nohup python train.py --task=${tasks[$i]} --seed=${seeds[$j]} --model_dir=./logs/${tasks[$i]}/ppo/ppo_seed$((${seeds[$j]}))/model_40000.pt --rl_device=cuda:$((2 + i % 6)) --sim_device=cuda:$((2 + i % 6)) --graphics_device_id=$((2 + i % 6)) --algo=ppo --headless --wandb_activate=True --wandb_entity=quantumiracle >> log/$DATE/${tasks[$i]}.log &	
	done
	wait
done
