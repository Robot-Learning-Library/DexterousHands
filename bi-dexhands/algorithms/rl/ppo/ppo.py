...
class PPO:
    def __init__(
        ...
        if reward_model:
            self.reward_model = torch.jit.load(
                f"/home/user/Documents/DexterousHands/bi-dexhands/reward_model/model_{frame_number}_gpu.pt",
        ...
        if len(self.learned_model) > 0:
            for i in range(len(self.learned_model)):
                self.other_primitive_actor_critic_list.append(
                    ActorCritic(
                        self.observation_space.shape,
                        self.state_space.shape,
                        self.action_space.shape,
            ...
            for i, actor_critic in enumerate(self.other_primitive_actor_critic_list):
                ...
                try:
                    ...
                    print(
                        "No learned model found under path: /ppo/ppo_seed{}/model_5000.pt".format(self.learned_model[i])
                    )
            ...
            for it in range(self.current_learning_iteration, num_learning_iterations):
                ...
                for _ in range(self.num_transitions_per_env):
                    ...
                    if self.reward_engineering:
                        ...
                        work_penalty = (
                            (
                                self.vec_env.task.dof_force_tensor
                                * self.vec_env.task.dof_state.view(self.vec_env.num_envs, -1, 2)[..., 1]
                            ).sum(-1)
                        ) ** 2
    ...
    def update(self):
        ...
        for epoch in range(self.num_learning_epochs):
            ...
            for indices in batch:
                ...
                for i, (other_actor_critic, valid) in enumerate(
                    zip(self.other_primitive_actor_critic_list, self.valid_primitive_list)
                ):
                    if valid:
                        other_actor_critic_actions, _, _, _, _ = other_actor_critic.act(obs_batch, states_batch)

                        other_actor_critic_actions_log_prob_batch, _, _, _, _ = self.actor_critic.evaluate(
                            obs_batch, states_batch, other_actor_critic_actions
                        )
                ...
                if self.policy_kl_reg and self.loaded_actor_critic is not None:
                    loaded_actor_critic_actions_log_prob_batch, _, _, _, _ = self.loaded_actor_critic.evaluate(
                        obs_batch, states_batch, actions_batch
                    )
