...
def plot_figure(
    ...
    **kwargs,
    ...
    if title:  # add title
        fig.suptitle(title, y = 0.93, fontsize=20)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--ylabel', default='Average Episode Success Rate', help='matplotlib figure ylabel'
    )
    parser.add_argument(
        '--shaded-std',
        action='store_true',
    ...
    input_root_dir = args.root_dir
    input_output_path = args.root_dir
...
'ShadowHandScissors', 'ShadowHandGraspAndPlace', 'ShadowHandSwitch', 'ShadowHandBottleCap', 'ShadowHandPen', 'ShadowHandLiftUnderarm']
    ...
    for task_name in task_envs:
        ...
        input_dir = args.root_dir  + "/" + task_name + "/RM"
        args.output_path = args.root_dir + "/" + task_name + "/RM/" + task_name + ".png"
        args.title = task_name[10:]
        if task_name == 'ShadowHand':
            args.title = "ShadowHand"
        if task_name == 'ShadowHandOver':
            args.title = "HandOver"
        file_lists = find_all_files(input_dir, re.compile(args.file_pattern))
        file_lists = [os.path.relpath(f, input_dir) for f in file_lists]
        if args.style:
