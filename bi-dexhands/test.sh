...
declare -a tasks=('ShadowHandLiftUnderarm')
...
for i in ${!tasks[@]}; do
	echo nohup python train.py --task=${tasks[$i]} --algo=ppo --test --record_traj=True --num_envs=1 --record_video_path=data/videos/test --max_iterations=10 --model_dir=./logs/${tasks[$i]}/ppo/ppo_seed-1/model_100000.pt --record_video=True --record_video_interval=1 : log/$DATE/${tasks[$i]}.log &
	...
	python -W ignore train.py  --task=${tasks[$i]} --algo=ppo --test --record_traj=True --num_envs=1 --record_video_path=data/videos/test1 --max_iterations=10 --model_dir=./logs/${tasks[$i]}/ppo/ppo_seed2/model_6500.pt --record_video=True --record_video_interval=1
done
