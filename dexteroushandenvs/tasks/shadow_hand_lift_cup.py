...
class ShadowHandLiftCup(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        shadow_hand_start_pose.p = gymapi.Vec3(0.55, 0.1, 0.75)
        shadow_hand_start_pose.r = gymapi.Quat().from_euler_zyx(0, 1.57, -1.57)

        shadow_another_hand_start_pose = gymapi.Transform()
        shadow_another_hand_start_pose.p = gymapi.Vec3(0.55, -0.1, 0.75)
        shadow_another_hand_start_pose.r = gymapi.Quat().from_euler_zyx(0, -1.57, -1.57)

        object_start_pose = gymapi.Transform()
        object_start_pose.p = gymapi.Vec3(0.0, 0., 0.65)
    ...
    def compute_observations(self):
        ...
        self.cup_right_handle_pos = self.cup_right_handle_pos + quat_apply(self.cup_right_handle_rot, to_torch([0, 1, 0], device=self.device).repeat(self.num_envs, 1) * -0.02)
        self.cup_right_handle_pos = self.cup_right_handle_pos + quat_apply(self.cup_right_handle_rot, to_torch([1, 0, 0], device=self.device).repeat(self.num_envs, 1) * 0.082)
        self.cup_right_handle_pos = self.cup_right_handle_pos + quat_apply(self.cup_right_handle_rot, to_torch([0, 0, 1], device=self.device).repeat(self.num_envs, 1) * 0.05)
        ...
        self.cup_left_handle_pos = self.cup_left_handle_pos + quat_apply(self.cup_left_handle_rot, to_torch([0, 1, 0], device=self.device).repeat(self.num_envs, 1) * 0.05)
        self.cup_left_handle_pos = self.cup_left_handle_pos + quat_apply(self.cup_left_handle_rot, to_torch([1, 0, 0], device=self.device).repeat(self.num_envs, 1) * -0.02)
        self.cup_left_handle_pos = self.cup_left_handle_pos + quat_apply(self.cup_left_handle_rot, to_torch([0, 0, 1], device=self.device).repeat(self.num_envs, 1) * 0.08)
...
def compute_hand_reward(
    ...
    up_rew = torch.where(goal_dist < 0.92,
                torch.where(right_hand_finger_dist < 0.5,
                    torch.where(left_hand_finger_dist < 0.5,
                        100*(0.92 - goal_dist), up_rew), up_rew), up_rew)    
    # reward = torch.exp(-0.1*(right_hand_dist_rew * dist_reward_scale)) + torch.exp(-0.1*(left_hand_dist_rew * dist_reward_scale))
    reward = 1 - right_hand_dist_rew - left_hand_dist_rew + up_rew
