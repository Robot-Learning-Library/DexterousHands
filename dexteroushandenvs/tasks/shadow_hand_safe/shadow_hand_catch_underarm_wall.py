...
class ShadowHandCatchUnderarmWall(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        # create wall asset
        self.wall_center_pos = [0, -0.575, 0.6]
        self.wall_outside_diameter = 1.0
        self.wall_inside_diameter = 0.1

        wall_dims = gymapi.Vec3((self.wall_outside_diameter - self.wall_inside_diameter) * 0.5 + self.wall_inside_diameter, 0.03, (self.wall_outside_diameter - self.wall_inside_diameter) * 0.5)
        asset_options = gymapi.AssetOptions()
        ...
        wall_pose1.p = gymapi.Vec3(self.wall_center_pos[0], self.wall_center_pos[1], self.wall_center_pos[2] + ((self.wall_outside_diameter - self.wall_inside_diameter) * 0.5 + self.wall_inside_diameter) * 0.5)
        wall_pose1.r = gymapi.Quat().from_euler_zyx(-0., 0, 0)
        wall_pose2 = gymapi.Transform()
        wall_pose2.p = gymapi.Vec3(self.wall_center_pos[0], self.wall_center_pos[1], self.wall_center_pos[2] - ((self.wall_outside_diameter - self.wall_inside_diameter) * 0.5 + self.wall_inside_diameter) * 0.5)
        wall_pose2.r = gymapi.Quat().from_euler_zyx(-0., 0, 0)
        wall_pose3 = gymapi.Transform()
        wall_pose3.p = gymapi.Vec3(self.wall_center_pos[0] + ((self.wall_outside_diameter - self.wall_inside_diameter) * 0.5 + self.wall_inside_diameter) * 0.5, self.wall_center_pos[1], self.wall_center_pos[2])
        wall_pose3.r = gymapi.Quat().from_euler_zyx(-0., 1.57, 0)
        wall_pose4 = gymapi.Transform()
        wall_pose4.p = gymapi.Vec3(self.wall_center_pos[0] - ((self.wall_outside_diameter - self.wall_inside_diameter) * 0.5 + self.wall_inside_diameter) * 0.5, self.wall_center_pos[1], self.wall_center_pos[2])
        ...
        self.wall_center_pos = to_torch(self.wall_center_pos, dtype=torch.float, device=self.device).repeat((self.num_envs, 1))
    ...
    def compute_reward(self, actions):
        ...
        # self.rew_buf[1:2], self.reset_buf[1:2], self.reset_goal_buf[1:2], self.progress_buf[1:2], self.successes[1:2], self.consecutive_successes[1:2] = compute_hand_reward(
        #     self.rew_buf[1:2], self.reset_buf[1:2], self.reset_goal_buf[1:2], self.progress_buf[1:2], self.successes[1:2], self.consecutive_successes[1:2],
        #     self.max_episode_length, self.object_pos[1:2], self.object_rot[1:2], self.goal_pos[1:2], self.goal_rot[1:2],
        #     self.dist_reward_scale, self.rot_reward_scale, self.rot_eps, self.actions[1:2], self.action_penalty_scale,
        #     self.success_tolerance, self.reach_goal_bonus, self.fall_dist, self.fall_penalty,
        #     self.max_consecutive_successes, self.av_factor, (self.object_type == "pen")
        # )
    ...
    def compute_cost(self, actions):
        ...
        self.cost_buf = torch.where(torch.abs(self.object_pos[:, 1] + 0.575) < 0.05,
                            torch.where(torch.norm((self.object_pos - self.wall_center_pos), p=2, dim=-1) > (self.wall_inside_diameter-0.03), torch.ones_like(self.cost_buf), self.cost_buf), self.cost_buf)
